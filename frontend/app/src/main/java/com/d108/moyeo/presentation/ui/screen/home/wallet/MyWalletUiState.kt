...
data class MyWalletUiState(
    // --- 데이터 관련 ---
    val walletInfo: BoxStoreUiState? = null, // BoxStore에서 가져올 지갑 정보
...
//필터 옵션을 위한 데이터 클래스
data class WalletFilterOptions(  // UI 문자열
...
fun FilterOptions.toWallet(): WalletFilterOptions {
    // 받은 FilterOptions가 Wallet용 어댑터가 맞는지 확인
    if (this !is WalletFilterOptionsAdp) error("Wallet 화면에서 처리할 수 없는 필터 타입")

    return WalletFilterOptions(
