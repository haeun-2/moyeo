...
sealed class StartDestination(val route: String) {
    ...
    object Login : StartDestination(AppScreen.Login.route) // AppScreen에 Login 경로가 정의되어 있다고 가정
}
...
class SplashScreenViewModel : ViewModel() {
    ...
    // 3. 최종 목적지 상태
    private val _startDestination = MutableStateFlow<StartDestination>(StartDestination.Login) // 기본값은 로그인
