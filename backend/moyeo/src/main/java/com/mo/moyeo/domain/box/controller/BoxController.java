...
public class BoxController {
    ...
    @GetMapping("/me")
    public ResponseEntity<BoxResponse> getMyBox() {
        BoxResponse response = boxService.getMyBox();
        return ResponseEntity.ok(response);
    }

    @Operation(summary = "모임 박스 목록 조회", description = "유저가 가입한 모임 박스 목록을 페이징하여 조회합니다.")
    @GetMapping
    public ResponseEntity<PageResponse<BoxResponse>> getGroupBoxList(@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "3") int size) {
        PageResponse<BoxResponse> response = boxService.getGroupBoxList(page, size);
        return ResponseEntity.ok(response);
    }

    @Operation(summary = "박스 권한 조회", description = "유저가 가진 박스의 이체/결제/환전 권한을 조회합니다.")
    @GetMapping("/{boxId}/permissions")
    public ResponseEntity<BoxPermissionResponse> getMyBoxPermissions(@PathVariable Long boxId) {
        BoxPermissionResponse response = boxService.getMyBoxPermissions(boxId);
        return ResponseEntity.ok(response);
    }

    @Operation(summary = "모임 박스 생성", description = "모임 박스의 기본 이름을 받아서 새로운 모임 박스를 생성합니다. 반화값은 생성된 모임 박스의 ID 입니다.")
    @PostMapping
    public ResponseEntity<BoxCreateResponse> createGroupBox(@Valid @RequestBody BoxCreateRequest request) {
        BoxCreateResponse response = boxService.createGroupBox(request);
        return ResponseEntity.ok(response);
    }
